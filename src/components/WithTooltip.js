...
class WithTooltip extends Component {
  ...
  componentDidMount() {
    if (!this.targetElement) {
      // Don't try and render the popper if Dom element has not been rendered
      // -- i.e during testing
      return;
    }
    document.body.addEventListener(
      'click',
      this.onHideIfOutsideTooltipOrCloseOnClick,
      false,
    );
    ...
    this.rootEl = document.getElementById('gitty-root') || document.body;
  ...
  onToggleHidden(event) {
    // Prevent default is used here because the events fire in different order on different platforms
    ...
    // On mobile, the react event fires *first* (most of the time), so preventDefault is used
        ...
        <Manager {...props}>
          ...
          >
            {typeof tooltip === 'function'
              ? tooltip({ onHide: this.onHideImmediately })
              : tooltip}
          </PopperWithArrow>
        </Manager>
      );
